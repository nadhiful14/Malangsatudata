<?php
class M_Diztribusi extends CI_Model
{
	...
	{
		$hsl = $this->db->query("UPDATE distribusibibit SET jenis_bibit='$jenisbibit',jumlah='$jumlah',periode='$periode',penginput='$penginput' WHERE id='$id'");
		return $hsl;
	}
	...
	public function tampil_jumlah($tahun)
	{
		$thn = $tahun;
		if ($thn == '0000') {
			$hsl = $this->db->query("select inputbibit.bibit,distribusibibit.jenis_bibit,distribusibibit.id, distribusibibit.jumlah,distribusibibit.periode from distribusibibit join inputbibit on inputbibit.id_bibit = distribusibibit.jenis_bibit where distribusibibit.status=0 ORDER BY periode desc

");
		} else {
			$hsl = $this->db->query("select inputbibit.bibit,distribusibibit.jenis_bibit,distribusibibit.id, distribusibibit.jumlah,distribusibibit.periode from distribusibibit join inputbibit on inputbibit.id_bibit = distribusibibit.jenis_bibit where distribusibibit.status=0 and distribusibibit.periode='$thn' ORDER BY periode desc");
		}
	...
	public function tampil_jumlahmax($tahun)
	{
		...
		$hsl = $this->db->query("SELECT inputbibit.bibit,distribusibibit.jenis_bibit,distribusibibit.id, sum(distribusibibit.jumlah) as jumlah,distribusibibit.periode from distribusibibit join inputbibit on inputbibit.id_bibit = distribusibibit.jenis_bibit where distribusibibit.status=0 and periode = '$thn' ORDER BY periode desc");
	...
	public function tampil_jumlahmaximum()
	{
		...
		$hsl = $this->db->query("SELECT inputbibit.bibit,distribusibibit.jenis_bibit,distribusibibit.id, sum(distribusibibit.jumlah) as jumlah,distribusibibit.periode from distribusibibit join inputbibit on inputbibit.id_bibit = distribusibibit.jenis_bibit where distribusibibit.status=0 ORDER BY periode desc");
	...
	{
		$hsl = $this->db->query("SELECT DISTINCT periode from distribusibibit where status='0' and periode BETWEEN '$tahun1' and '$tahun2' order by periode asc");
		return $hsl;
	}
