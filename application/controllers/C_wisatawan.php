...
class C_wisatawan extends CI_Controller
{
  ...
  public function tambah_detail_wisatawan_datang()
  {
    ...
    $cek = $this->db->query("SELECT * FROM detail_wisatawan where bulan='$bulan' and tahun='$tahun' AND jenis_wisatawan='wisatawan_datang' and status = 0");
    if ($cek->num_rows() >= 1) {
      echo "<script>alert('Maaf data sudah di inputkan.');</script>";
      redirect('C_wisatawan/tampil_detail_wisatawan_datang/' . $page, 'refresh');
    } else {
