...
class C_wisatawan_menginap extends CI_Controller {
    ...
    public function tambah_detail_wisatawan_menginap(){
        ...
        $cek = $this->db->query("SELECT * FROM detail_wisatawan where bulan='$bulan' and tahun='$tahun' AND jenis_wisatawan='wisatawan_menginap' and status = 0");
           if ($cek->num_rows()>=1)
           {
            echo "<script>alert('Maaf data sudah di inputkan.');</script>";
            redirect('C_wisatawan_menginap/tampil_detail_wisatawan_menginap/'.$page,'refresh');
           }
           else {

          $this->m_wisatawan_menginap->simpan_wisatawan_datang($bulan, $tahun, $domestik, $mancanegara, $penginput, $jenis_wisatawan);
        ...
        $cek = $this->db->query("SELECT * FROM detail_wisatawan where bulan='$bulan' and tahun='$tahun' AND jenis_wisatawan='wisatawan_menginap' and status = 0");
           ...
           {
            echo "<script>alert('Maaf data sudah di inputkan.');</script>";
            redirect('C_wisatawan_menginap','refresh');
           }
           else {
